...
var options = {
    ...
    // Call when one image is appear to the viewport. Params are the appear image element node,count of elements not appear yet, and the options.
    appear             : null, // default: undefined. Another example: function(element, elements_left, options){}
    // Call when one image is load. Params are the appear image element node,count of elements not appear yet, and the options.
    load               : null,
